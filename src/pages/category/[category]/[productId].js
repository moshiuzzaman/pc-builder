...
export const getStaticPaths = async () => {
  const res = await fetch("https://assignment6server.vercel.app/products");
  const products = await res.json();

  const paths = products.map((product) => ({
    params: {category: product.category, productId: product._id },
  }));
...
export async function getStaticProps({ params }) {
  ...
  // Replace the following with your actual logic to fetch product data
  const productRes = await fetch(
    `https://assignment6server.vercel.app/products?_id=${productId}`
  ...
  return {
    ...
    revalidate: 10, // Adjust revalidate time as needed
  };
}
